...
const upsertSession = (rows: SessionInfo[], session: SessionInfo): SessionInfo[] => {
  ...
  return [session, ...rows.filter(row => !isSame(row))].sort(compareProjectRank)
}
...
export function overlayLivePreviews(
  ...
  for (const node of projects) {
    ...
    // Project pins then lead (newest first), then recency. The user's explicit
    // sort key (rankIds) still runs after this and wins when set: an explicit
    // sort is a stronger statement than a pin.
    const pool = uniqueConversations([...liveRows, ...base]).sort(compareProjectRank)
